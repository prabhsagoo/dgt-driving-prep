    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-black text-white tracking-tight">
            {copy.title[language]}
          </h1>
        ...
        {mistakeQuestions.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-slate-900 px-3.5 py-2.5 text-xs font-semibold text-rose-400 hover:bg-rose-500/10 transition-all"
            >
      ...
      {mistakeQuestions.length === 0 ? (
        /* Empty State */
        <div className="rounded-3xl border border-white/10 bg-slate-900/50 p-12 text-center backdrop-blur-2xl shadow-xl">
          ...
          <h2 className="mt-4 text-xl font-bold text-white">{copy.emptyTitle[language]}</h2>
          <p className="mt-1.5 text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
            {copy.emptySubtitle[language]}
          </p>
          <Link
            href="/exam"
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-amber-400 px-6 py-3 text-xs font-bold text-slate-950 hover:bg-amber-300 transition-all shadow-lg shadow-amber-500/10"
          >
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-6 sm:p-8 backdrop-blur-xl shadow-2xl space-y-6">
          ...
          <div className="space-y-3">
            {currentOptions.map((opt, optIdx) => {
              ...
              let style = 'border-white/5 bg-slate-800/40 text-slate-300 hover:border-white/20 hover:bg-slate-800/80';
              if (isAnswered) {
                if (isCorrect) {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl border transition-all flex items-start gap-3.5 ${style}`}
                >
          ...
          {isAnswered && (
            <div className={`p-4 rounded-2xl border text-xs leading-relaxed animate-fadeIn ${
              ...
              <p className="text-slate-300 mt-2 border-t border-white/5 pt-2">
                <strong className="text-white">{copy.explanationLabel[language]}: </strong>
                {currentExplanation}
              </p>
            </div>
          )}
          ...
          <div className="flex items-center justify-between border-t border-white/5 pt-5">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-slate-800/60 px-4 py-2.5 text-xs font-semibold text-slate-300 hover:bg-white/10 disabled:opacity-30 disabled:pointer-events-none transition-all"
            >
            ...
            {isAnswered ? (
              <button
                onClick={handleNext}
                className="inline-flex items-center gap-1.5 rounded-xl bg-amber-400 px-5 py-2.5 text-xs font-bold text-slate-950 hover:bg-amber-300 transition-all"
              >
              ...
              <button
                onClick={() => handleNext()}
                className="inline-flex items-center gap-1.5 rounded-xl bg-white/5 px-4 py-2 text-xs font-semibold text-slate-400 hover:text-white transition-all"
              >
                {copy.skip[language]}
              </button>
            )}
        ...
        <div className="space-y-4">
          {mistakeQuestions.map((q, index) => {
            ...
            return (
              <div
                key={`${q.id}-${index}`}
                className="rounded-2xl border border-white/10 bg-slate-900/60 p-5 backdrop-blur-xl shadow-md transition-all hover:border-white/20"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-rose-500/10 text-rose-400 font-bold text-xs border border-rose-500/20">
                      {index + 1}
                    </span>
                ...
                <div className="mt-3 ml-9 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 text-xs text-emerald-300">
                  <strong className="text-emerald-400">{copy.correctAnswerLabel[language]} </strong>
                  {correctOpt}
                </div>

                <p className="mt-2 ml-9 text-xs text-slate-400 italic">
                  {explanation}
                </p>
              </div>
            );
